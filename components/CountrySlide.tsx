...
const CountrySlide: React.FC<Props> = ({ data }) => {
  return (
    <div className="h-full w-full relative overflow-hidden flex flex-col md:flex-row">
      ...
      {/* Content Side */}
      <div className="w-full md:w-1/2 h-1/2 md:h-full bg-slate-950 p-6 md:p-12 flex flex-col justify-center relative z-10">
        ...
        >
          ...
          {/* Description */}
          <p className="text-lg md:text-xl text-slate-300 mb-8 leading-relaxed font-light opacity-90">
            {data.description}
          </p>
          ...
          <motion.div 
            ...
            className="bg-white/5 backdrop-blur-sm border-r-4 border-l-0 border-white/10 p-6 rounded-2xl relative mb-8 hover:bg-white/10 transition-colors duration-300"
          >
             <Quote className={`absolute -top-4 -right-4 text-${data.color}-500/40 fill-current